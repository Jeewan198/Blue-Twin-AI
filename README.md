...
### 2. Generate the train/test split
This happens automatically the first time any script calls `river_split.py`'s `get_train_test_split()` — there is no separate script to run. It's fully deterministic (fixed seed), so every script always agrees on the same 949 training rivers and 129 held-out test rivers.
...
### 4. Evaluate against the static baseline
```bash
python evaluate_agent.py
```
**Requires**: a trained checkpoint from step 3.
...
### 5. Verify behaviour against ecological constraints
```bash
python inspect_agent_actions.py
```
**Requires**: the same trained checkpoint as step 4.
